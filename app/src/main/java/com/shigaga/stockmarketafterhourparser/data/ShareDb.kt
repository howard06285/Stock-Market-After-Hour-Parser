 ...
 */
@Database(entities = arrayOf(Shares::class), version = 1)
abstract class ShareDb : RoomDatabase() {
    abstract fun shareDao(): SharesDao

    companion object {
        private var instance: ShareDb? = null
        @Synchronized
        fun get(context: Context): ShareDb {
            if (instance == null) {
                instance = Room.databaseBuilder(context.applicationContext,
                        ShareDb::class.java, "mDatabase")
                        .build()
            }
            return instance!!
        }
